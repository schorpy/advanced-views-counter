...
export default function AgentNew() {
  ...
  const isNewCampaign = path === '/agents/new'
  const id = !isNewCampaign ? path.split('/').pop() : null

  const fetchCampaignData = async () => {
    if (isNewCampaign || !id) return null

    try {
      setIsLoading(true)
      const response = await fetch(`${chatizy.pluginApiUrl}/agent/get/${id}`, {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    document.title = id ? 'Copy Campaign' : 'New Campaign'

    const loadCampaign = async () => {
      if (!isNewCampaign) {
        setCampaignType('copy')
  ...
  if (isLoading) {
    return (
      <AgentsLayout breadcrumb={breadcrumb}>
        <div role="status" className="flex items-center justify-center min-h-[300px]">
