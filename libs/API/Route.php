...
class Route
{
	...
	private const METHOD_PUT    = array('PUT', 'PATCH');
	...
	private function put_normal($endpoint, $callback, $auth = false)
	{
		$prefix = $this->current['prefix'];
	...
	private function delete_normal($endpoint, $callback, $auth = false)
	{
		$prefix = $this->current['prefix'];
		...
		foreach (self::$routes as $prefix) {
			foreach ($prefix as $route) {
				...
				$args = array(
					...
					'permission_callback' => $auth ?: '__return_true',
					'callback'            => $callback,
				);
